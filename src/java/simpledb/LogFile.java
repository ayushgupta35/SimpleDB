    ...
    public void rollback(TransactionId tid)
      throws NoSuchElementException, IOException {
        ...
        synchronized (this) {
            preAppend();
            if (!undo(tid.getId())) {
                throw new NoSuchElementException("transaction " + tid + " never started");
            }
        }
        }
    }

    private boolean undo(long tid) throws IOException {
        Long first_rec = tidToFirstLogRecord.get(tid);
        if (first_rec == null) {
            return false;
        }
        raf.seek(first_rec);
    
        HashMap<PageId, Page> allRollbacks = new HashMap<>();
        boolean hasAborted = false;
    
        while (true) {
            try {
                int type = raf.readInt();
                long rec_tid = raf.readLong();
    
                if (type == BEGIN_RECORD) {
                } else if (type == COMMIT_RECORD) {
                    if (tid == rec_tid) {
                        throw new RuntimeException("Saw COMMIT");
                    }
                    ...
                    if (rec_tid == tid && !allRollbacks.containsKey(before.getId())) {
                        allRollbacks.put(before.getId(), before);
                    }
        ...
        for (Map.Entry<PageId, Page> entry : allRollbacks.entrySet()) {
        ...
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                ...
                if (location != -1) {
                    ...
                    raf.readLong();
                    if (cpType != CHECKPOINT_RECORD) {
                        throw new RuntimeException("Checkpoint pointer does not point to the correct checkpoint record");
                    }
                ...
                while (true) {
                    try {
                        ...
                        long tidRec = raf.readLong();
    
                        switch (recordType) {
                            case CHECKPOINT_RECORD:
                                throw new RuntimeException("Saw CHECKPOINT");
                            case BEGIN_RECORD:
                                if (tidToFirstLogRecord.containsKey(tidRec)) {
                                    throw new RuntimeException("Duplicate BEGIN");
                                }
                                tidToFirstLogRecord.put(tidRec, cur);
                                break;
                            case ABORT_RECORD:
                                if (!tidToFirstLogRecord.containsKey(tidRec)) {
                                    throw new RuntimeException("Oorphan ABORT");
                                }
                                long offset = raf.getFilePointer();
                                undo(tidRec);
                                raf.seek(offset);
                                tidToFirstLogRecord.remove(tidRec);
                                break;
                            case COMMIT_RECORD:
                                if (!tidToFirstLogRecord.containsKey(tidRec)) {
                                    throw new RuntimeException("Orphan COMMIT");
                                }
                                tidToFirstLogRecord.remove(tidRec);
                                break;
                            case UPDATE_RECORD:
                                if (!tidToFirstLogRecord.containsKey(tidRec)) {
                                    throw new RuntimeException("Orphan UPDATE_RECORD");
                                }
                                Page before = readPageData(raf);
                                Page after = readPageData(raf);
                ...
                ArrayList<Long> allIDs = new ArrayList<>(tidToFirstLogRecord.keySet());
                for (Long id : allIDs) {
